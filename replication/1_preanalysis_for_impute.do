...
trajplot, xtitle(Year) ytitle(Score)  ci legendLabels(label(1 "Persistently strained") label(2 "Steadily strengthening") label(3 "Rapidly strengthening") label(4 "Gradual decline w/ sust. positivity") size(small) rows(2) colgap(*2)) 
...
save "$data\group_traj_3wave_forimput" , replace
...
use "$data\data_afterdrop_forimput" , clear
merge m:1 pid using "$data\group_traj_3wave_forimput" , nogen keep(1 3)  

g outcome_dep = depression if num==max_num
gsort pid outcome_dep
carryforward outcome_dep , replace

foreach cov in page female hk_ru mar peduy hp pinc iadl depression numchilds year lowses {
...
save "$data\finaldata_ir_dep_forimput", replace
